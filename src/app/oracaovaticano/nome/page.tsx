...
export default function OracaoVaticanoNomePage() {
  const [name, setName] = useState('');
  const router = useRouter();

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    router.push(`/oracaovaticano/quiz3?name=${encodeURIComponent(name)}`);
  };
          ...
          <Card className="bg-white/80 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-2xl border border-blue-200">
            <CardContent className="space-y-6 text-blue-950">
              ...
              <p className="text-center text-blue-900/90">
                Deixe seu nome abaixo para que possamos te mandar as orações diretamente para você 🙏
              </p>
