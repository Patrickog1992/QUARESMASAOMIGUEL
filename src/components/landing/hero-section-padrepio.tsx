'use client';

export function HeroSectionPadrePio() {
  return (
    <section className="space-y-4">
      <h1 className="text-3xl md:text-5xl font-bold uppercase text-amber-300 drop-shadow-[0_2px_2px_rgba(0,0,0,0.7)]">
        AS ORAÇÕES SECRETAS DO PADRE PIO QUE ESTÃO FAZENDO MILAGRES
      </h1>
      <p className="text-lg md:text-xl font-medium text-amber-100">
        Receba sua benção de saúde, prosperidade e vida financeira através das preces do Santo Padre Pio.
      </p>
       <p className="font-bold text-amber-300 uppercase tracking-wider animate-pulse md:text-lg">
            Assista ao vídeo abaixo e receba a sua bênção
        </p>
    </section>
  );
}
